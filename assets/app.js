...
function isSessionPending() {
    return !!currentUser && currentUser.isSessionPending === true;
}
...
function hasPremiumAccess() {
    // Chỉ cấp quyền từ user đã được backend trả về sau login/restoreSession.
    if (!currentUser || currentUser.isGuest || isSessionPending()) return false;
...
function requirePremium(featureName) {
    ...
    if (isSessionPending()) {
        showLoadingOverlay('Đang chờ xác thực phiên đăng nhập...');
        setTimeout(() => hideLoadingOverlay(), 1200);
        restoreSessionFromToken({ silent: true });
        return false;
    }
...
async function restoreSessionFromToken({ silent = false } = {}) {
    if (!accountSessionToken || sessionRestoreInProgress) return false;
    ...
    if (!silent) showLoadingOverlay('Đang khôi phục phiên đăng nhập...');

    try {
        const res = await callAppsScript('restoreSession', {});

        if (res.ok && res.student) {
            currentUser = { ...res.student, isGuest: false, isSessionPending: false };
            enterDashboard(true);
            return true;
        }
        ...
        // Lỗi mạng tạm thời: giữ nguyên token, không chuyển về Khách.
        currentUser = makePendingSessionUser();
        enterDashboard(true);
        return false;
    } finally {
    ...
    if (isSessionPending()) {
        box.innerHTML = `
            <div class="flex items-center gap-2">
                <span class="text-indigo-600 font-extrabold text-xs"><i class="fa-solid fa-cloud-arrow-down mr-1"></i>Đang khôi phục phiên...</span>
            </div>`;
...
function updatePremiumLockDecorations() {
    ...
    if (!locked) return;
    const selectors = ['[onclick^="openTopic(11,"]', '[onclick="openExamHub()"]'];
    selectors.forEach(sel => {
        grid.querySelectorAll(sel).forEach(card => {
            card.style.position = 'relative';
            const badge = document.createElement('span');
            badge.setAttribute('data-premium-lock-badge', '1');
            badge.className = 'absolute top-2 right-2 text-slate-400 text-xs';
...
async function loadAdminStudents(showBusy = false) {
    ...
    try {
        ...
        alert('Không thể tải danh sách tài khoản: ' + e.message);
...
function openRoadmap() {
    if (!requirePremium('Bản đồ tuần')) return;
    stopSpeaking();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('online', () => {
        if (accountSessionToken && isSessionPending()) {
            restoreSessionFromToken({ silent: true });
        }
    });
});
